...
const BRAND_INTRO_SLIDES = [
  { id: 'cover', layout: 'cover' },
  {
    ...
    pill: 'What it is',
    headline: 'The operating system a business runs on.',
    paragraphs: [
      "Brand Systems is the foundational work: identity, voice, visual system, deck templates, sales enablement tools, and campaign concepts. Built once, it accelerates everything that comes after. Every asset you produce gets faster, more consistent, and more credible.",
      "It's not a one-off project. It's infrastructure. The design system handles the heavy lifting. The voice guide sets the standard. The deck template system means your team ships polished work without starting from scratch.",
    ],
  ...
  },
  {
    ...
    when: "When you're entering a new market, pitching to enterprise buyers, or any time first impressions determine whether you get a second meeting.",
    before: "Mismatched logos, inconsistent colors, and a deck built from a template. Prospects form a judgment before you get to slide three.",
    after: "Every touchpoint looks like it came from the same place: website, deck, email signature, one-pager. Trust is established before the conversation starts.",
    stats: [
      { metric: 'Brand consistency', result: '+10–20% revenue lift (Lucidpress)' },
      { metric: 'Prospect-to-meeting conversion', result: '+15–25%' },
      { metric: 'First-impression trust', result: '3–4× more likely to achieve strong visibility' },
    ],
  },
  {
    id: 'outcome-deals', layout: 'outcome-detail',
    pill: 'Deal size',
    headline: 'Close bigger deals.',
    when: "When you're moving upmarket, selling to procurement committees, or competing against larger, better-funded rivals.",
    before: "Deals lost not because the product was wrong but because the materials didn't match the ask. Decision-makers feel uncertain.",
    after: "Sales reps show up with materials that reinforce the value of what they're selling. Buyers feel confident recommending you internally.",
    stats: [
      { metric: 'Win rate', result: '+15–20% with strong sales enablement (Salesforce)' },
      { metric: 'Average deal size', result: '+10–15% ACV lift' },
      { metric: 'Sales cycle length', result: '20–30% shorter with polished materials' },
    ],
  },
  {
  ...
  },
  {
    ...
    when: "Any time you're in a raise: seed, Series A, or beyond. The quality of your materials reflects directly on the quality of your thinking.",
    before: "A deck built in a hurry the week before investor meetings. Inconsistent visuals. A story that doesn't flow. Investors pick up on it.",
    after: "Materials that look like they belong in the room. A narrative that's been crafted, not assembled. Investors spend time on the content, not the presentation.",
    stats: [
  ...
  {
    id: 'packages', layout: 'bs-packages',
    pill: 'Packages',
    packages: [
      { name: 'Brand Pilot', goal: 'Low-risk first project', includes: ['Brand strategy', 'Brand sprint'], price: '$14,350' },
      { name: 'Verbal Identity Kit', goal: 'Name, voice, and messaging framework', includes: ['Naming & verbal identity', 'Copy audit & voice guide'], price: '$7,550' },
      { name: 'Brand Modernisation', goal: 'Bring an established brand back in line', includes: ['Brand strategy', 'Brand refresh', 'Deck template system'], price: '$27,210' },
      { name: 'Sales Deck System', goal: 'Reusable deck system with template kit', includes: ['Deck strategy & narrative', 'Sales deck — full', 'Deck template system'], price: '$14,660' },
      { name: 'Enterprise Sales Toolkit', goal: 'Branded sales enablement for high-stakes rooms', includes: ['Deck strategy & narrative', 'Sales deck — full', 'Case study film', 'One-pager / checklist'], price: '$25,895' },
      { name: 'Funding Round Pack', goal: 'Investor-ready story and materials', includes: ['Deck strategy & narrative', 'Investor / pitch deck', 'Brand refresh'], price: '$28,970' },
      { name: 'Market Credibility System', goal: 'A brand that makes you credible to bigger clients', includes: ['Brand strategy', 'Brand system', 'Deck template system'], price: '$41,680' },
    ...
    areas: [
      {
        name: 'Identity',
        services: [
          { name: 'Brand strategy', price: '$3,810' },
          { name: 'Brand sprint', price: '$10,540' },
          { name: 'Brand system', price: '$32,050' },
          { name: 'Brand platform', price: '$54,300' },
          { name: 'Brand refresh', price: '$17,580' },
          { name: 'Naming & verbal identity', price: '$4,815' },
          { name: 'Copy audit & voice guide', price: '$2,735' },
        ],
      },
      {
        name: 'Sales & Decks',
        services: [
          { name: 'Deck strategy & narrative', price: '$1,730' },
          { name: 'Sales deck — short', price: '$2,755' },
          { name: 'Sales deck — full', price: '$7,110' },
          { name: 'Investor / pitch deck', price: '$9,660' },
          { name: 'Keynote / event deck', price: '$8,725' },
          { name: 'Deck template system', price: '$5,820' },
          { name: 'Board / leadership deck', price: '$4,880' },
        ],
      },
      {
        name: 'Campaign Concepts',
        services: [
          { name: 'Campaign strategy & concepts', price: '$2,360' },
          { name: 'Concept sprint', price: '$7,500' },
          { name: 'Campaign concept', price: '$11,170' },
          { name: 'Pitch / new business concept', price: '$9,440' },
        ],
      },
      {
        name: 'Print & Physical',
        services: [
          { name: 'Brand book / printed report', price: '$10,200' },
          { name: 'Editorial / magazine spread', price: '$6,545' },
          { name: 'Packaging — single SKU', price: '$7,355' },
          { name: 'Packaging — product line', price: '$15,460' },
          { name: 'Print ad campaign', price: '$7,330' },
          { name: 'OOH campaign', price: '$10,970' },
        ],
      },
      {
        name: 'AI for Brand',
        services: [
          { name: 'AI brand training & guidelines', price: '$5,680' },
        ],
      },
    ],
  },
...
export default function BrandSystems() {
  ...
  const clients = (data ?? [])
    ...
    .filter(p => p.tiles.length > 0 && (p.summary || p.tagline) && BRAND_SLUGS.has(p.slug))

  const pool = clients.flatMap(c => c.tiles)

  const introSlides = BRAND_INTRO_SLIDES.map(slide => ({ kind: 'intro', slide }))
  ...
  useMeta({
    title: 'Brand Systems | Super Conscious',
    description: 'Identity, voice, visual system, and sales enablement. Built once, used everywhere.',
    path: '/brand-systems',
  })
...
function IntroSlide({ slide, tile }) {
  switch (slide.layout) {
    ...
    case 'capability-areas':  return <CapabilityAreasSlide slide={slide} />
    case 'outcomes':          return <BsOutcomesSlide slide={slide} />
  ...
  return (
    <section className={styles.coverSlide}>
      ...
      <svg
        ...
        aria-hidden="true"
      >
        <g clipPath="url(#sc-mark-clip-brand)">
        ...
        <defs>
          <clipPath id="sc-mark-clip-brand">
      ...
      <div className={styles.coverCornerStack}>
        ...
        <div className={styles.coverCorner}>Identity, voice, visual system, and sales enablement. Built once, used everywhere.</div>
      </div>
    </section>
  )
}
...
function CapabilityAreasSlide({ slide }) {
  return (
    <section className={styles.introSlide}>
      <Pill>{slide.pill}</Pill>
      <div className={styles.bsCapGrid}>
        {slide.areas.map(area => (
          <div key={area.name} className={styles.bsCapCard}>
            <p className={styles.bsCapName}>{area.name}</p>
            <p className={styles.bsCapDesc}>{area.desc}</p>
            <div className={styles.servicesTags}>
              {area.services.map(s => (
                <span key={s} className={styles.servicesTag}>{s}</span>
              ))}
            </div>
          </div>
        ))}
...
function BsOutcomesSlide({ slide }) {
  return (
    <section className={styles.introSlide}>
      <Pill>{slide.pill}</Pill>
...
function BsOutcomeDetailSlide({ slide, tile }) {
  return (
    <section className={styles.introSlide}>
      <Pill>{slide.pill}</Pill>
...
function BsPackagesSlide({ slide }) {
...
function BsServicesSlide({ slide }) {
  return (
    <section className={styles.introSlide}>
      <Pill>{slide.pill}</Pill>
