...
export default function AgencyCapabilities() {
  ...
  const introSlides = AGENCY_INTRO_SLIDES.map(slide => ({ kind: 'intro', slide }))
  ...
  useMeta({
    title: 'Agency Capabilities | Super Conscious',
    description: 'A white-label production studio for agencies — brand systems, content programs, and digital products.',
    path: '/agency-capabilities',
  })
...
const AGENCY_INTRO_SLIDES = [
  { id: 'cover', layout: 'cover' },
  {
    id: 'who-we-are', layout: 'who-we-are',
    pill: 'Who we are',
    headline: 'Your embedded production studio.',
    paragraphs: [
      "We're a small team of designers, animators, and engineers who sit inside your agency as a silent production partner. We handle the build — brand systems, content programs, and digital products — while you own the client relationship, start to finish.",
      "No handoff lag. No offshore quality issues. No conflict of interest. We work under your name, within your process, and on your timeline.",
    ],
  ...
  },
  {
    ...
    pill: 'What we produce',
    headline: 'What we produce.',
    cards: [
      {
  ...
  },
  {
    ...
    pill: 'Why it works',
    headline: 'What the partnership gives you.',
    cards: [
      {
        tag: 'Capacity',
        title: 'Take on more without hiring.',
        body: "Win bigger scopes without adding headcount. We add production capacity you can flex up or down based on your active pipeline — no long-term employment risk, no ramp time.",
        source: 'Agency model',
      },
      {
        tag: 'Quality',
        title: 'Deliver better work.',
        body: "Elevate what you're shipping. Production-ready brand systems, award-caliber content, engineered digital products — delivered on brief and ready to go out under your name.",
        source: 'Our work',
      },
      {
        tag: 'Margin',
        title: 'Protect your relationships.',
        body: 'We stay invisible to your clients. No direct contact, no credit, no conflict. You stay the trusted partner and keep the full margin on the work we produce.',
        source: 'White-label model',
      },
    ],
  },
  {
    ...
    pill: 'How we work',
    headline: 'White-labeled, not credited.',
    items: [
      { num: '01', title: 'Fully white-labeled.', body: "We don't touch the client relationship. No emails, no calls, no Slack with your clients unless you explicitly want us to. The work goes out under your name — always." },
      { num: '02', title: 'We plug into your process.', body: "Your PM tools, your naming conventions, your delivery format. We adapt to how you work, not the other way around. Minimum onboarding friction, maximum output from day one." },
      { num: '03', title: 'Scale when you need it.', body: "Month-to-month, project-by-project, or retainer. We staff to match your active pipeline. Add capacity fast when you win big, pull back when things are quiet. No long-term headcount risk." },
    ],
  },
  {
  ...
  },
  {
    ...
    pill: 'Partner rates',
    headline: 'Transparent pricing built for agency margins.',
    body: 'Every scope is built from the same per-role rate card. Full visibility into what goes into every line item — so you can mark up cleanly and confidently.',
    rates: [
  ...
  {
    id: 'commitment', layout: 'commitment',
    pill: 'Partner commitment',
    headline: 'The longer you commit, the better the rate.',
    tiers: [
      {
        label: 'Month-to-month',
        rate: '$146 / hr',
        body: 'Full flexibility. No minimum term. Best for a first project or when you need to move fast before committing to more.',
      },
      {
        label: '3-month',
        rate: '$135 / hr',
        body: 'A quarter of predictable production. Enough runway to build something meaningful and get a working rhythm in place.',
      },
      {
        label: '6-month',
        rate: '$120 / hr',
        body: 'The partner rate. Committed pipeline lets us staff more efficiently, and we pass the savings directly to you.',
      },
    ],
  },
]
...
function IntroSlide({ slide, cardTiles, mosaicTiles }) {
  switch (slide.layout) {
    ...
    case 'commitment': return <CommitmentSlide slide={slide} />
    default: return null
  }
}
  ...
  return (
    <section className={styles.coverSlide}>
      ...
      <svg
        ...
        aria-hidden="true"
      >
        <g clipPath="url(#sc-mark-clip-agency)">
        ...
        <defs>
          <clipPath id="sc-mark-clip-agency">
      ...
      <div className={styles.coverCornerStack}>
        <div className={styles.coverCorner}>
          A white-label production studio for agencies.
        </div>
        <div className={styles.coverCorner}>
        ...
        {slide.cards.map((card, i) => {
          ...
          return (
            <div key={card.name} className={styles.offeringCard}>
              {tile && <Tile tile={tile} className={styles.offeringImage} />}
      ...
      <div className={styles.numberedLayout}>
        ...
        {variant && tiles.length > 0 && (
          <div className={styles.numberedMosaic}>
            {tiles.map((tile, i) => (
              <Tile key={i} tile={tile} className={`${styles.mosaicTile} ${styles[`tile_${variant[i].shape}`]}`} />
            ))}
          </div>
        )}
      </div>
...
function CommitmentSlide({ slide }) {
  return (
    <section className={styles.introSlide}>
      <Pill>{slide.pill}</Pill>
      <div className={styles.commitmentTiers}>
        {slide.tiers.map(tier => (
          <div key={tier.label} className={styles.commitmentTier}>
            <p className={styles.commitmentLabel}>{tier.label}</p>
            <p className={styles.commitmentRate}>{tier.rate}</p>
...
const STATIC_CATEGORY_MAP = {
  'arbitrum-rebrand':              [{ key: 'brand',   achievement: 'Built a brand identity ambitious enough for the leading Ethereum Layer 2.' }],
  'transcend':                     [{ key: 'brand',   achievement: 'Crafted a breakthrough neuroscience brand ready for clinical and consumer markets.' }],
  'gigs':                          [{ key: 'brand',   achievement: 'Built a trust-first brand for a platform professionalizing the gig economy.' }],
  'industry-standard':             [{ key: 'brand',   achievement: 'Rebuilt a denim brand identity from the ground up — visual system, voice, and all.' },
                                    { key: 'content', achievement: 'Launched an organic content program to grow community and drive repeat purchase.' }],
  'wonderwerk':                    [{ key: 'brand',   achievement: 'Gave a California wellness brand a visual identity with real energy and edge.' }],
  'banzen':                        [{ key: 'brand',   achievement: 'Built a dispensary brand that makes cannabis feel premium, approachable, and local.' }],
  'tbt':                           [{ key: 'brand',   achievement: 'Brought scientific credibility to a clinical-stage dermatology company through brand design.' }],
  'world-within':                  [{ key: 'brand',   achievement: 'Built a brand with the emotional weight needed to carry a mission-driven platform.' },
                                    { key: 'content', achievement: 'Developed a content program to convert awareness into community and donations.' }],
  'smallhold':                     [{ key: 'brand',   achievement: 'Established mushroom as a premium, culturally relevant ingredient through brand-building.' },
                                    { key: 'content', achievement: 'Built a content engine that made Smallhold a household name in specialty produce.' }],
  'photon':                        [{ key: 'brand',   achievement: 'Built a credible, design-led brand for a platform rewriting the prescription network.' },
                                    { key: 'product', achievement: 'Designed the product UI for a system that moves prescriptions faster and more cleanly.' }],
  'heard':                         [{ key: 'product', achievement: 'Built the experience layer for an AI-powered platform that makes user research faster.' }],
  'print-parlor':                  [{ key: 'brand',   achievement: 'Repositioned a merch company as a design-led business with an identity to match.' },
  ...
  'arbitrum-openhouse':            [{ key: 'product', achievement: 'Built a launchpad site for the Arbitrum builder community, designed to drive event sign-ups.' }],
  'youtube':                       [{ key: 'content', achievement: 'Produced a short-form series that connected the YouTube creator ecosystem.' }],
  'path-projects':                 [{ key: 'content', achievement: 'Created a motion content system that made a performance apparel brand look and move differently.' }],
  'arbitrum-marketing-dept-videos':[{ key: 'content', achievement: 'Turned crypto conversation into shareable video content across Arbitrum\'s owned channels.' }],
  'google-ads':                    [{ key: 'content', achievement: 'Produced a short-form series giving quick takes on the platforms shaping modern marketing.' }],
  'google-champions-place':        [{ key: 'content', achievement: 'Made a documentary for Google that brought community and place-based sport to life.' }],
  'google-language-ai':            [{ key: 'content', achievement: 'Humanized AI with a documentary series about the people building language AI at Google.' }],
  'survivornet':                   [{ key: 'content', achievement: 'Told one survivor\'s mission in a documentary built to inspire and mobilize.' }],
  'google-maps':                   [{ key: 'content', achievement: 'Documented the people behind Google Maps in a series that made the invisible visible.' }],
  'spot-and-tango':                [{ key: 'content', achievement: 'Captured real New York through its dog owners in a short-form social series for Google.' }],
  'google-small-business':         [{ key: 'content', achievement: 'Told the stories of small businesses the way they deserved to be told, for Google.' }],
  'google-think-with-google':      [{ key: 'content', achievement: 'Produced an interview series capturing conversations with the people shaping modern marketing.' }],
  'google':                        [{ key: 'content', achievement: 'Produced documentary and short-form content for Google across multiple programs.' }],
}
