...
// roughly 16:9 in a typical slide, so 1×1 reads as ~16:9 and 1×2 reads
// near-square. Shapes: small (1×1), tall (1×2), hero (2×2), column (1×3).
const VARIANTS = {
  4: [
...
export default function Capabilities() {
  ...
  // Intro slides first, then one slide per client.
  const introSlides = INTRO_SLIDES.map(slide => ({ kind: 'intro', slide }))
  const clientSlides = clients.map(client => ({ kind: 'client', client }))
  const slides = [...introSlides, ...clientSlides, { kind: 'closing' }]
  // Offering slide gets representative tiles per card, drawn from the
  // case study media pool. 4 tiles per card × 3 cards = 12 distinct tiles.
  const pool = clients.flatMap(c => c.tiles)

  const offeringIdx = slides.findIndex(s => s.kind === 'intro' && s.slide.id === 'what-we-do')
  if (offeringIdx !== -1) {
    slides[offeringIdx] = {
      ...slides[offeringIdx],
      cardTiles: [pool[0], pool[1], pool[2]],
    }
  }
  ...
  useMeta({
    ...
    description: 'The studio at work — what we do, how we work, and the brands we build and grow.',
    path: '/capabilities',
  })
  ...
  if (total === 0) return (
    <main className={styles.main}>
      <p className={styles.notFound}>No published projects with enough media yet.</p>
    </main>
  )
...
const INTRO_SLIDES = [
  ...
  {
    id: 'who-we-are', layout: 'who-we-are',
    pill: 'Who we are',
    headline: 'The makers and purveyors of high quality brands, content, and digital products.',
    paragraphs: [
      "We are a small studio of designers, animators, and engineers. We make brand systems, content programs, and digital products for founders and marketing teams who care about how their company shows up, and who want a creative partner who'll sit on the inside of the team, not the outside.",
  ...
  {
    id: 'disciplines', layout: 'disciplines',
    pill: 'Disciplines',
    headline: 'The crafts we put on every project.',
    items: [
  ...
  {
    id: 'services', layout: 'services',
    pill: 'Services',
    headline: 'What you can buy.',
    columns: [
      {
        name: 'Brand Systems',
        tag: 'The foundation.',
        groups: [
          { name: 'Identity', items: ['Brand strategy', 'Brand sprint', 'Brand system', 'Brand platform', 'Brand refresh', 'Naming & verbal identity', 'Copy audit & voice guide'] },
          { name: 'Decks', items: ['Deck strategy & narrative', 'Sales deck', 'Investor / pitch deck', 'Keynote deck', 'Deck template system'] },
          { name: 'Campaigns & concepts', items: ['Campaign strategy', 'Concept sprint', 'Campaign concept', 'Pitch concept'] },
          { name: 'Print & OOH', items: ['Brand book', 'Editorial spread', 'Packaging', 'Print ads', 'OOH campaign'] },
          { name: 'AI for brand', items: ['AI brand training & guidelines'] },
        ],
      },
      {
        name: 'Digital Products',
        tag: 'The build.',
        groups: [
          { name: 'Product design', items: ['Product discovery', 'UX wireframes', 'UI design', 'Full product UI', 'Design-to-build', 'Prototyping'] },
          { name: 'Web builds', items: ['Landing page', 'Marketing site', 'Marketing site + CMS', 'Big marketing site + CMS', 'Shopify store', 'Microsite', 'Design system'] },
          { name: 'Interactive & AI', items: ['Interactive / WebGL', 'Calculator / quiz', 'Custom AI tool / agent'] },
        ],
      },
      {
        name: 'Content Programs',
        tag: 'The fuel.',
        groups: [
          { name: 'Social', items: ['Social strategy', 'Static posts', 'Carousels', 'Short-form video', 'Animated loops', 'Campaign launch pack', 'Multi-platform launch'] },
          { name: 'Long-form video', items: ['Video strategy', 'Brand film (:30)', 'Brand film (:60–:90)', 'Documentary short', 'Founder story', 'Case study film', 'Product launch film', 'TVC + cutdowns', 'Sizzle reel'] },
          { name: 'Email & newsletter', items: ['Email strategy', 'Email batches', 'Newsletter template', 'Campaign email series', 'Editorial newsletter'] },
          { name: 'Lifecycle & retention', items: ['Lifecycle email program', 'Customer onboarding', 'Loyalty / retention campaign'] },
          { name: 'Ad creative', items: ['Ad concept', 'Static ad sets', 'Video ad sets', 'Display ad campaign', 'Always-on ad production'] },
          { name: 'Editorial & lead magnets', items: ['Content strategy', 'One-pager', 'Short guide', 'Ebook / playbook', 'Industry report', 'Lead magnet bundle'] },
          { name: 'Copy & scripts', items: ['Campaign copy', 'Website copy', 'Long-form content', 'Scriptwriting'] },
        ],
      },
    ],
  },
  {
    id: 'packages', layout: 'packages',
    pill: 'Packages',
    headline: 'Bundled scopes, fixed price.',
    columns: [
      {
        name: 'Brand Systems',
        tag: 'The foundation.',
        items: [
          ...
          { name: 'Funding Round Pack', goal: 'Investor-ready story and materials that hold up under scrutiny.', price: 'From $28,000' },
          { name: 'Market Credibility System', goal: 'A brand that makes you credible to bigger clients, partners, and investors.', price: 'From $41,000' },
        ],
      },
      {
        name: 'Digital Products',
        ...
        items: [
          ...
          { name: 'Product Build', goal: 'Designed, built product in front of real users. Discovery through working build.', price: 'From $36,000' },
          { name: 'Full Web Build', goal: 'Large multi-template marketing site with a design system to extend it.', price: 'From $43,000' },
        ],
      },
      {
        name: 'Content Programs',
        ...
        items: [
          ...
          { name: 'GTM Launch Pack', goal: 'Bring a product or moment to market with one coordinated push.', price: 'From $54,000' },
          { name: 'Awareness Campaign', goal: 'Get in front of people who don\'t know you yet and become a name they recognise.', price: 'From $54,000' },
        ],
      },
    ],
  },
  {
    id: 'how-we-price', layout: 'pricing',
    pill: 'How we price',
    headline: 'We price services and packages from an hourly rate card.',
    body: 'Every line item and every bundle is built from the same per-role rates. Fully visible, aligned upfront.',
    rates: [
      { role: 'Writer / Strategist', rate: '$200 / hr' },
      { role: 'Creative Director', rate: '$175 / hr' },
      { role: 'Executive Producer', rate: '$175 / hr' },
      { role: 'Developer', rate: '$175 / hr' },
      { role: 'Illustration', rate: '$175 / hr' },
      { role: '3D Design & Motion', rate: '$160 / hr' },
      { role: 'Animator', rate: '$150 / hr' },
      { role: 'Editor', rate: '$150 / hr' },
      { role: 'Associate Creative Director', rate: '$140 / hr' },
      { role: 'Lead Designer', rate: '$125 / hr' },
      { role: 'Designer', rate: '$95 / hr' },
      { role: 'Project Manager', rate: '$95 / hr' },
      { role: 'Jr. Designer', rate: '$80 / hr' },
    ],
    blended: { role: 'Blended average', rate: '$146 / hr' },
  },
]

function IntroSlide({ slide, cardTiles, mosaicTiles }) {
  switch (slide.layout) {
    case 'cover': return <CoverSlide />
    case 'prose': return <ProseSlide slide={slide} />
  ...
  return (
    <section className={styles.coverSlide}>
      ...
      <div className={styles.coverCornerStack}>
        <div className={styles.coverCorner}>
          A creative production and development studio.
        </div>
        <div className={styles.coverCorner}>
          Brand systems, content programs, and digital products.
        </div>
      </div>
    </section>
  )
...
function WhoWeAreSlide({ slide }) {
  return (
    <section className={styles.whoWeAreSlide}>
      <video
        className={styles.whoWeAreVideo}
        src={slide.videoUrl}
        autoPlay
        loop
        muted
...
function ProseSlide({ slide }) {
  return (
    <section className={styles.introSlide}>
      <Pill>{slide.pill}</Pill>
      <h2 className={styles.proseHeadline}>{slide.headline}</h2>
    ...
    <section className={styles.introSlide}>
      ...
      <div className={styles.threeCol}>
        {slide.cards.map(card => (
          <div key={card.name} className={styles.compareCard}>
            <p className={styles.compareName}>{card.name}</p>
            <p className={styles.compareBody}>{card.body}</p>
          </div>
        ))}
      </div>
      <p className={styles.slideFooter}>{slide.footer}</p>
    </section>
  )
}

function OfferingSlide({ slide, cardTiles }) {
  return (
    <section className={styles.introSlide}>
      <Pill>{slide.pill}</Pill>
      <div className={styles.offeringStack}>
        {slide.cards.map((card, i) => {
          const tile = card.staticImage ? { src: card.staticImage, isVideo: card.staticIsVideo ?? false } : cardTiles[i]
          return (
            <div key={card.name} className={styles.offeringCard}>
              {tile && (
                <Tile tile={tile} className={styles.offeringImage} />
              )}
              <div className={styles.offeringCardText}>
                <p className={styles.offeringName}>{card.name}</p>
                <p className={styles.offeringTag}>{card.tag}</p>
                <p className={styles.offeringBody}>{card.body}</p>
              </div>
            </div>
          )
        })}
...
function OutcomesSlide({ slide }) {
  return (
    <section className={styles.introSlide}>
      ...
      <div className={styles.threeCol}>
        {slide.cards.map(card => (
          <div key={card.tag} className={styles.outcomeCard}>
            <p className={styles.outcomeTag}>{card.tag}</p>
            <p className={styles.outcomeTitle}>{card.title}</p>
            <p className={styles.outcomeBody}>{card.body}</p>
            <p className={styles.outcomeSource}>Source: {card.source}</p>
          </div>
        ))}
      </div>
    </section>
...
function DisciplinesSlide({ slide }) {
  return (
    <section className={styles.introSlide}>
      <Pill>{slide.pill}</Pill>
      <div className={styles.disciplinesGrid}>
        {slide.items.map(d => (
          <div key={d.name} className={styles.disciplineCard}>
...
function NumberedSlide({ slide, mosaicTiles }) {
  const variantCount = pickVariant(mosaicTiles.length)
...
function ServicesSlide({ slide }) {
  return (
    <section className={styles.introSlide}>
      <Pill>{slide.pill}</Pill>
      <div className={styles.servicesPillars}>
        {slide.columns.map(col => (
          <div key={col.name} className={styles.servicesPillar}>
            <div className={styles.servicesPillarHead}>
              <p className={styles.servicesPillarName}>{col.name}</p>
              <p className={styles.servicesPillarTag}>{col.tag}</p>
            </div>
            <div className={styles.servicesTags}>
              {col.groups.flatMap(g => g.items).map(item => (
                <span key={item} className={styles.servicesTag}>{item}</span>
              ))}
            </div>
          </div>
        ))}
...
function PackagesSlide({ slide }) {
  return (
    <section className={styles.introSlide}>
      <Pill>{slide.pill}</Pill>
      <div className={styles.packagesGrid}>
        {slide.columns.map(col => (
          <div key={col.name} className={styles.servicesCol}>
            <div className={styles.servicesColHead}>
              <p className={styles.servicesColName}>{col.name}</p>
              <p className={styles.servicesColTag}>{col.tag}</p>
            </div>
...
function PricingSlide({ slide }) {
  const mid = Math.ceil(slide.rates.length / 2)
  const left = slide.rates.slice(0, mid)
  const right = slide.rates.slice(mid)
  return (
    <section className={styles.introSlide}>
      <Pill>{slide.pill}</Pill>
      {slide.body && <p className={styles.pricingLead}>{slide.body}</p>}
      <div className={styles.rateCard}>
        <div className={styles.rateColumns}>
          {[left, right].map((col, ci) => (
            <ul key={ci} className={styles.rateCol}>
              {col.map(r => (
                <li key={r.role} className={styles.rateRow}>
                  <span className={styles.rateRole}>{r.role}</span>
                  <span className={styles.rateValue}>{r.rate}</span>
                </li>
              ))}
            </ul>
          ))}
        ...
        {slide.blended && (
          <div className={styles.rateBlended}>
            <p className={styles.rateBlendedLabel}>{slide.blended.role}</p>
            <p className={styles.rateBlendedValue}>{slide.blended.rate}</p>
          </div>
        )}
      </div>
    </section>
...
function SelectedWorkSlide({ slide, thumbnails }) {
  return (
    <section className={styles.introSlide}>
      <Pill>{slide.pill}</Pill>
      <h2 className={styles.headline}>{slide.headline}</h2>
      <div className={styles.selectedGrid}>
        {thumbnails.slice(0, 12).map(client => {
          const tile = client.tiles?.[0]
          return (
            <div key={client._id} className={styles.selectedTile}>
              {tile?.isVideo
                ? <LazyVideo src={tile.src} className={styles.tileMedia} />
                : tile && <img src={`${tile.src}?w=800&q=80&auto=format`} alt="" loading="lazy" className={styles.tileMedia} />}
              <span className={styles.selectedName}>{client.name}</span>
            </div>
          )
        })}
      </div>
...
// Static fallback until category/achievement is set in Sanity.
// Each entry: array of { key, achievement } per pillar.
const STATIC_CATEGORY_MAP = {
  'arbitrum-rebrand': [
    { key: 'brand', achievement: 'Built a brand identity ambitious enough for the leading Ethereum Layer 2.' },
  ],
  'transcend': [
    { key: 'brand', achievement: 'Crafted a breakthrough neuroscience brand ready for clinical and consumer markets.' },
  ],
  'gigs': [
    { key: 'brand', achievement: 'Built a trust-first brand for a platform professionalizing the gig economy.' },
  ],
  'industry-standard': [
    { key: 'brand', achievement: 'Rebuilt a denim brand identity from the ground up — visual system, voice, and all.' },
    { key: 'content', achievement: 'Launched an organic content program to grow community and drive repeat purchase.' },
  ],
  'wonderwerk': [
    { key: 'brand', achievement: 'Gave a California wellness brand a visual identity with real energy and edge.' },
  ],
  'banzen': [
    { key: 'brand', achievement: 'Built a dispensary brand that makes cannabis feel premium, approachable, and local.' },
  ],
  'tbt': [
    { key: 'brand', achievement: 'Brought scientific credibility to a clinical-stage dermatology company through brand design.' },
  ],
  'world-within': [
    { key: 'brand', achievement: 'Built a brand with the emotional weight needed to carry a mission-driven platform.' },
    { key: 'content', achievement: 'Developed a content program to convert awareness into community and donations.' },
  ],
  'smallhold': [
    { key: 'brand', achievement: 'Established mushroom as a premium, culturally relevant ingredient through brand-building.' },
    { key: 'content', achievement: 'Built a content engine that made Smallhold a household name in specialty produce.' },
  ],
  'photon': [
    { key: 'brand', achievement: 'Built a credible, design-led brand for a platform rewriting the prescription network.' },
    { key: 'product', achievement: 'Designed the product UI for a system that moves prescriptions faster and more cleanly.' },
  ],
  'heard': [
    { key: 'product', achievement: 'Built the experience layer for an AI-powered platform that makes user research faster.' },
  ],
  'print-parlor': [
    { key: 'brand', achievement: 'Repositioned a merch company as a design-led business with an identity to match.' },
    { key: 'product', achievement: 'Built a clean, conversion-focused site that turned browsing into buying.' },
  ],
  'yura': [
    { key: 'brand', achievement: 'Designed a brand that makes AI-powered benefits feel approachable and trustworthy.' },
    { key: 'product', achievement: 'Built a digital benefits platform that changes how employees access their coverage.' },
  ],
  'arbitrum-openhouse': [
    { key: 'product', achievement: 'Built a launchpad site for the Arbitrum builder community, designed to drive event sign-ups.' },
  ],
  'youtube': [
    { key: 'content', achievement: 'Produced a short-form series that connected the YouTube creator ecosystem.' },
  ],
  'path-projects': [
    { key: 'content', achievement: 'Created a motion content system that made a performance apparel brand look and move differently.' },
  ],
  'arbitrum-marketing-dept-videos': [
    { key: 'content', achievement: 'Turned crypto conversation into shareable video content across Arbitrum\'s owned channels.' },
  ],
  'google-ads': [
    { key: 'content', achievement: 'Produced a short-form series giving quick takes on the platforms shaping modern marketing.' },
  ],
  'google-champions-place': [
    { key: 'content', achievement: 'Made a documentary for Google that brought community and place-based sport to life.' },
  ],
  'google-language-ai': [
    { key: 'content', achievement: 'Humanized AI with a documentary series about the people building language AI at Google.' },
  ],
  'survivornet': [
    { key: 'content', achievement: 'Told one survivor\'s mission in a documentary built to inspire and mobilize.' },
  ],
  'google-maps': [
    { key: 'content', achievement: 'Documented the people behind Google Maps in a series that made the invisible visible.' },
  ],
  'spot-and-tango': [
    { key: 'content', achievement: 'Captured real New York through its dog owners in a short-form social series for Google.' },
  ],
  'google-small-business': [
    { key: 'content', achievement: 'Told the stories of small businesses the way they deserved to be told, for Google.' },
  ],
  'google-think-with-google': [
    { key: 'content', achievement: 'Produced an interview series capturing conversations with the people shaping modern marketing.' },
  ],
  'google': [
