...
export const BACKLOG = [
  {
    ...
    items: [
      ...
      { name: 'Date picker', state: 'proto', why: 'Any booking or deadline field — a text date gets a different format from every visitor.' },
      { name: 'Combobox', state: 'proto', why: 'A select with too many options to show: clients, projects, tags.' },
      { name: 'Switch', state: 'proto', why: 'A setting that takes effect immediately, unlike a checkbox that waits for submit.' },
      { name: 'Stepper', state: 'proto', why: 'Nudging a number without selecting and retyping it.' },
      { name: 'Tag input', state: 'proto', why: 'Multi-value entry — disciplines, skills, filters.' },
      { name: 'Slider', state: 'proto', why: 'Already inside before/after and the video scrub; not yet a shared control.' },
    ],
  },
  {
    group: 'Navigation',
    ...
    items: [
      ...
      { name: 'Skip link', state: 'proto', why: 'Absent today, so a keyboard user traverses the whole rail on every page.' },
      { name: 'Prev / next', state: 'proto', why: 'A case study is currently a dead end — nothing links one to the next.' },
      { name: 'Scrollspy', state: 'proto', why: 'Long posts and deck pages. The chip nav exists but never says where you are.' },
      { name: 'Sidebar', state: 'proto', why: 'The rail works at eight links and breaks at twenty.' },
      { name: 'Command palette', state: 'proto', why: 'Ninety-six routes. The only navigation here that scales without a redesign.' },
    ],
  },
  {
    group: 'Content',
    ...
    items: [
      ...
      { name: 'Data grid', state: 'proto', why: 'Anything sortable or addressable by cell — a spreadsheet, not a styled table.' },
      { name: 'Callout', state: 'proto', why: 'A note inside prose that is not a quote.' },
      { name: 'Definition list', state: 'proto', why: 'A glossary or a spec — term and definition, paired.' },
      { name: 'Timeline', state: 'proto', why: 'Editorial process or studio history, undated and in order.' },
      { name: 'Avatar / person', state: 'proto', why: 'About and Careers both want one. Nothing exists in the codebase.' },
    ],
  },
  {
    group: 'Feedback',
    ...
    items: [
      ...
      { name: 'Inline banner', state: 'proto', why: 'No .alert, .banner or .notice class exists anywhere. Page-level notices have nowhere to live.' },
      { name: 'Spinner', state: 'proto', why: 'A wait of unknown length, where a progress bar would have to lie.' },
    ],
  },
  {
    group: 'Media',
  ...
  {
    group: 'Overlays',
    note: 'Both drawers ship without role="dialog", aria-modal or a focus trap. The primitive underneath them does not exist.',
    items: [
      { name: 'Modal', state: 'proto', why: 'The primitive the drawer and confirm dialog both need and neither has.' },
      { name: 'Drawer', state: 'have', why: 'Cal and contact ship. Undocumented, and missing dialog semantics.' },
      { name: 'Dropdown menu', state: 'proto', why: 'No action menu exists in the codebase. A select returns a value; a menu performs a verb.' },
      { name: 'Focus trap', state: 'proto', why: 'Needed by every overlay above. Restore-on-close is the half most implementations skip.' },
      { name: 'Popover', state: 'gap', why: 'Anchored, non-modal content — a filter panel or a definition.' },
      { name: 'Bottom sheet', state: 'gap', why: 'The mobile form of the drawer, if the drawer ever needs one.' },
    ],
  },
  {
    group: 'AI',
